...
class TaskService:
    ...
    def get_tasks_by_status(
        self, status: TaskStatus, owner_id: str
    ) -> list[TaskResponse]:

        tasks = (
            self.db.query(Task)
            .filter(Task.status == status, Task.owner_id == owner_id)
            .all()
        )
    ...
    def get_task_by_priority(
        self, priority: TaskPriority, owner_id: str
    ) -> list[TaskResponse]:
        tasks = (
            self.db.query(Task)
            .filter(Task.priority == priority, Task.owner_id == owner_id)
            .all()
        )
    ...
    def update_task(
        self, task_id: str, task_update: TaskUpdate, owner_id: str
    ) -> TaskResponse:
        try:
            task = (
                self.db.query(Task)
